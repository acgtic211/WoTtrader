...
export class DiscoverService {
  ...
  getDevices(apiUrl = this.apiUrl, search: string, hopLimit: number = -1, timeLimit: number = -1, typeOfSearch: string = 'broadcast') {
    ...
    return this.http.get(`${apiUrl}/search?content=${search}${query}`);
  }

  getDevicesJSON(apiUrl = this.apiUrl, search: string, hopLimit: number = -1, timeLimit: number = -1, typeOfSearch: string = 'broadcast') {
    console.log(apiUrl)
    var query = "";
    if(hopLimit >= 0) {
      query += `&hopLimit=${hopLimit}`;
    ...
    return this.http.get(`${apiUrl}/search/jsonpath?query=${search}${query}`);
  }

  getDevicesSPARQL(apiUrl = this.apiUrl, search: string) {
    console.log(apiUrl)

    return this.http.get(`${apiUrl}/search/sparql?query=${search}`);
  }
}